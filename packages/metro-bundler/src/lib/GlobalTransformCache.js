...
function profileKey(_ref) {let dev = _ref.dev,minify = _ref.minify,platform = _ref.platform;
...
class URIBasedGlobalTransformCache {
  ...
  /**
               ...
               * megabytes each.
               */
  static _fetchResultFromURI(uri) {return _asyncToGenerator(function* () {
      const response = yield fetch(uri, { method: 'GET', timeout: 8000 });
  ...
  /**
               ...
               */
  static _fetchResultFromURIWithRetry(uri) {
  ...
  /**
               ...
               */
  fetch(props) {var _this5 = this;return _asyncToGenerator(function* () {
      const uri = yield _this5._fetcher.fetch(_this5.keyOf(props));
      if (uri == null) {
        return null;
      }
  ...
  store(props, result) {var _this6 = this;return _asyncToGenerator(function* () {
      if (_this6._store != null) {
        yield _this6._store.store(_this6.keyOf(props), result);
      }})();
  }}
...
class OptionsHasher {
  ...
  options)
  {const
    dev = options.dev,minify = options.minify,platform = options.platform,transform = options.transform,unknowns = _objectWithoutProperties(options, ['dev', 'minify', 'platform', 'transform']);
  ...
  {const
    ...
    options.enableBabelRCLookup,generateSourceMaps = options.generateSourceMaps,dev = options.dev,hot = options.hot,inlineRequires = options.inlineRequires,platform = options.platform,projectRoot = options.projectRoot,unknowns = _objectWithoutProperties(options, ['enableBabelRCLookup', 'generateSourceMaps', 'dev', 'hot', 'inlineRequires', 'platform', 'projectRoot']);
    const unknownKeys = Object.keys(unknowns);
    if (unknownKeys.length > 0) {
